...
{
    public class DisableStorageDeviceTest
    {
        [Test]
        public void Test()
        {
            ...
            // Clicking on the Storage Settings Link and disabling a device
            var editStorageSettingsLink = new StorageDevicesPage();
            PageFactory.InitElements(driver, editStorageSettingsLink);
            WebDriverWait storageDevicesLinkWait = new WebDriverWait(driver, TimeSpan.FromSeconds(10));
            IWebElement storageDevicesLinkButton = storageDevicesLinkWait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementToBeClickable(By.CssSelector("[href*='#/domains/Audio/encoders/HDR700_Rec/storage/devices/settings']")));
            editStorageSettingsLink.StorageSettings();
            driver.Manage().Timeouts().ImplicitWait = TimeSpan.FromSeconds(10);
            editStorageSettingsLink.DisableEnableStorageDevice();
            driver.Manage().Timeouts().ImplicitWait = TimeSpan.FromSeconds(10);
            editStorageSettingsLink.Submit();
